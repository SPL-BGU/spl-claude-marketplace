...
**Make it domain-specific.** This is a general-purpose specialist. To turn it into a sharper expert for a recurring domain (plugin development, database tuning, a specific framework), copy `agents/specialist.md` to a new name and fill in the domain's authoritative sources, canonical examples, and decision rules. For a fully worked example, see the live `plugin-specialist` agent in the marketplace repo (`.claude/agents/plugin-specialist.md`) — a research-driven Claude Code plugin / MCP-server specialist specialized from this very agent.
